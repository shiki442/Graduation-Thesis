...
\end{theorem}
\begin{proof}
首先证明第一部分。当 $\mu<+\infty$ 时，对泛函\eqref{eq:conloss} 沿任意测试函数 $v \in H^1(\Omega)$ 求变分，可得其 Euler-Lagrange 方程的弱形式：
\begin{equation*}
\begin{aligned}
...
\end{theorem}
\begin{proof}
根据泛函 $J^{\mu}$ 的定义\eqref{eq:conloss}，我们可以将 $J^{\mu}(u^{\mu}+v)$ 展开并与 $J^{\mu}(u^{\mu})$ 作差，得到：
\begin{equation*}
\begin{aligned}
...
\end{itemize}
综合上述四项，严格得出结论：$J(u^*) \le \liminf_{k \to \infty} J(u_k)$。
$$J(u^*) \le \liminf_{k \to \infty} J(u_k)$$

第四步，证明解的存在性。结合第二、三步的结论，我们有：
$$J(u^*) \le \liminf_{k \to \infty} J(u_k) = \lim_{k \to \infty} J(u_k) = m$$
又因 $m = \inf J(u)$，必有 $J(u^*) \ge m$。故 $J(u^*) = m$，这表明 $u^{*} \in H^1(\Omega)$ 为能量泛函的全局极小值点，即弱解存在。
...
Dirichlet边界条件的证明与Robin边界条件类似。
定义函数空间 $\mathcal{A}$ 为满足Dirichlet边界条件的函数集合：
$$\mathcal{A} = \left\{ u \in H^1(\Omega) \mid u = h \text{ on } \partial \Omega \text{ (在迹的意义下)} \right\}$$
定义泛函 $J: \mathcal{A} \to \mathbb{R}$ 为：
$$J(u) = \frac{1}{2}\int_{\Omega} |\nabla u|^2 \, dx + \int_{\Omega} W(u) \, dx - \int_{\Omega} gu \, dx$$
其中 $W(s) = \int_0^s V(t) \, dt$。我们将泛函分为二次项 $Q(u)$、非线性项 $N(u)$ 以及线性项 $L(u)$。与Robin边界条件的情况相比，$Q(u)$ 和 $L(u)$少了关于$\mu$的边界项。

令$u^{*} = \arg\min_{u \in \mathcal{A}} J(u)$，通过计算 $J(u)$ 的 Gâteaux 变分可知，$u^{*}$ 即为方程 \eqref{eq:maineq} 在Dirichlet边界条件下的弱解。类似地，可以分步证明解的存在性、唯一性与正则性。我们再次只指出与Robin边界条件证明不同的部分。
第一步：证明泛函 $J(u)$ 的强制性。
对于任意 $u \in \mathcal{A}$，令 $v = u - h$。显然 $v=0$ 在边界上成立，因此 $v \in H^1_0(\Omega)$
根据 $H^1_0$ 空间上的庞加莱不等式：
