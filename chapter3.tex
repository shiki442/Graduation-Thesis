...
注意到 $\mathcal{P}_{\leq 1}\delta u$ 满足方程\eqref{eq:3.16}，我们有：
...
\State {计算瑞利商与边界惩罚项:
$$J_{base} = \frac{|\Omega|}{N}\sum_{i=1}^{N} \left[ \frac{|\nabla v_{\phi}(X_{i})|^2}{2}+\frac{w(X_{i})v_{\phi}^{2}(X_{i})}{2}\right] + \frac{\mu_{1}}{2}\frac{|\partial\Omega|}{M}\sum_{i=1}^{M} T v_{\phi}^{2}(Y_{i})$$}
\State {计算特征函数归一化约束项:
...
\end{equation*}
并记 $\alpha=\frac{m-2}{d}$。假设定理 \ref{mainthm} 中误差上界的三项具有一致的衰减速率 $\widetilde{\mathcal{O}}(n^{-r})$，可推导出如下参数关系：
...
\end{aligned}\right.
\end{equation}
其中 $\Omega=[0,1]^d$ 且 $w(t)=\exp(-\pi t)$（指数势）。方程 \eqref{eq:conloss} 是一个变量分离方程，其解在每个维度上都表现出各向同性。可以通过利用有限元方法导出参考解。

在本节中，我们首先考虑几种低维情形，并研究计算误差如何随 $\varepsilon_1$、$\varepsilon_2$、神经网络规模以及样本量变化，并与理论结果相对应。本节中所有实验的默认设置如下：$d=2$，学习率 = 0.001，$\varepsilon_1=1000$，$\varepsilon_2=200$，$\mathcal{W} = 100$，$\mathcal{D} = 4$，epoch $= 10000$，样本量 $= 10000/3000$。我们将以此作为各种对照实验的基准线。
\begin{figure}[htbp]
    \centering
...
在本节及下一节中，我们在式\eqref{eq:conloss} 中选取了不同的函数 $w(x)$，包括常用的函数，如指数函数 $(w(x)=\exp(-\pi x))$ 和平方函数 $(w(x)=x^2)$。此外，我们探索了不同的区域，包括 10 维立方体和 10 维球体。在这些区域上，依赖于空间离散化的传统数值算法（如有限元法和有限差分法）一直无法提供令人满意的结果。实验的参数设置如下：
