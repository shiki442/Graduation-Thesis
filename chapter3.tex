\chapter{DRM求解椭圆多特征值问题}\label{chap:eigen}
本章聚焦于高维空间下椭圆型多重特征值问题的深度学习求解方法.针对传统数值方法在高维困境下的局限性，本章开发了一种基于DRM的系统求解算法.该算法不仅致力于实现多重特征值与特征函数的求解，还建立了一套独立于空间维数的先验误差分析理论，从数学本质上论证该方法克服“维数灾难”的潜力.
\section{问题背景与预备知识}\label{sec_pre}
\subsection{特征值问题概述}
椭圆型特征值问题在数学物理中占据核心地位，是描述微观量子能级概率分布与宏观结构振动模态的基石.然而，当物理模型向复杂高维空间拓展时，诸如有限元法等传统网格离散方法会遭遇明显的”维数灾难”，其计算成本显著增长且逼近精度急剧下降.为突破这一计算瓶颈，本章基于该问题的惩罚变分原理，构造了一种适用于同步求解多重特征对的通用无约束优化泛函，并采用DRM对其进行高效逼近.

在理论层面，本章严格推导了网络输出特征值与真实值之间误差的非渐近上界.该理论界限由前馈神经网络的深度 $\mathcal{D}$、宽度 $\mathcal{W}$ 以及无网格训练样本量 $n$ 共同显式表达，为深度神经求解器的收敛性提供了理论支撑.

此外，为验证理论可靠性与算法泛化能力，本章开展了最高达20维的高维数值实验.在传统离散方法难以有效适用的场景下，本文构建的神经求解器能够将前几阶特征值的相对误差控制在 $10^{-2}$ 量级，并较好地捕捉对应激发态的本征模态特征.

本章的组织结构如下：第\ref{sec_pre}节概述全文所需的基本假设、预备知识与符号系统；第\ref{sec_model}节详细推导多重特征值问题的惩罚变分公式及神经网络求解框架；第\ref{sec_main}节提供核心的先验误差分析与收敛率证明；第\ref{sec_num}节通过数值实验验证算法在高维场景下的有效性与理论分析的客观性；最后，在第\ref{sec dis}节总结全章.
\subsection{数学描述和谱定理}
本文主要研究如下带有Dirichlet边界条件的椭圆型特征值问题：
...
根据椭圆偏微分方程的经典理论，上述微分算子 $\mathcal{L} = -\Delta + V$ 在配备 Dirichlet 边界条件的 Hilbert 空间 $L^2(\Omega)$ 上是自伴的，且由于区域 $\Omega$ 的有界性，其逆算子（或格林算子）$\mathcal{L}^{-1}$ 是紧算子.这一性质使得我们可以直接应用泛函分析中的紧自伴算子谱定理：

\begin{theorem}[谱分解定理\cite{evans2010partial}]
...
对于椭圆型特征值问题\eqref{eq:eigendiriproblem}，结论如下成立\cite{evans2010partial}：
\begin{enumerate}
...
\quad
\begin{enumerate}
    \item \textbf{谱投影算子}：基于算子 $\mathcal{L}$ 的特征值分解，定义谱投影算子 $\mathcal{P}_{<k}$ 为：
    \begin{equation*}
    ...
    \item \textbf{特征值分离度}：定义特征值 $\lambda_i$ 的分离度为：
    \begin{equation*}
...
需要特别注意的是，由于特征值可能存在重数，即特征空间的维数可能大于 1，上述定义中不同的索引 $k$ 可能对应相同的特征值.这也是我们在公式 \eqref{eq:eigenvalue_sequence} 中使用非严格不等号 ($\leq$) 的原因.

特征值问题的正则性是本文分析的另一个核心要素.足够高的正则性对于避免高维问题中的”维数灾难”具有重要意义（详见第 \ref{sec_main} 节）.为此，我们提出如下假设：
...
本文引入上述较强的区域正则性假设，核心目的在于确保特征函数具备高阶连续性.这使得我们能够调用高阶空间的神经网络逼近定理，进而推导出更优的理论收敛速率.在低正则性条件下，亦可借助标准的逼近理论建立类似的收敛性保证，尽管其理论收敛阶可能有所降低.

此外，很多几何情形虽然并不满足上述假设，但只要能证明特征函数的高阶连续性，本文核心结论仍然成立.例如，$\mathbb{R}^{d}$ 中的单位立方体虽然在角点处不满足 $C^m$ 边界条件，但若势函数为常数或齐次势，我们的定理结论显然依然成立.针对此类非光滑区域的有效性，我们将在第 \ref{sec_num} 节通过数值算例进一步验证.

\section{算法设计与分析}
\subsection{特征值与瑞利商}\label{sec_model}
针对前文公式 \eqref{eq:eigendiriproblem} 定义的椭圆型特征值问题，其变分形式自然对应于瑞利商的极值问题.为此，我们首先定义如下的双线性能量泛函：

\begin{equation*}
a(u,v)\triangleq\langle \nabla u,\nabla v\rangle+\int_{\Omega}V\cdot uv \text{d}x, u\in H_{0}^{1}(\Omega).
\end{equation*}
...
\end{equation}
对于高阶特征值，我们可以采用递归变分法进行定义.第 $k$ 个特征值 $\lambda_k$ 对应于瑞利商在与前 $k-1$ 个特征函数正交的子空间上的最小值.即：
...
虽然理论上我们可以直接基于经典的DRM，利用公式 \eqref{eq:ktheigenvalue} 来求解第 $k$ 个特征值及其对应的特征函数，但在实际计算中，该方法的直接应用面临着以下三个主要的本质困难：
...
针对一般区域的 PDE 问题，边界惩罚法提供了一种更为灵活的“软约束”策略 \cite{duan2022convergence, Mller2021ErrorEF}.其核心思想是引入如下修正的双线性形式：
...
\item 第二个困难源于瑞利商泛函本身的几何性质.瑞利商具有尺度不变性（零次齐次性），即对于任意非零常数 $c$，有 $R(c u) = R(u)$.这意味着优化目标在特征函数的径向方向上是常数，导致解空间呈现出流形结构而非单一极小值点，明显削弱了梯度的导向性.
更为重要的是，这种非凸性导致优化过程在原点附近较为不稳定.当近似解的模长趋于零（$\|v\| \to 0$）时，瑞利商的梯度计算将出现奇异性，导致优化问题变得病态.为克服这一平凡解风险，必须在损失函数中引入适当的正则化项或稳定项\cite{Weinan2017The}，以约束解的模长并维持数值稳定性.

\item 求解高阶特征值（$k > 1$）面临着额外的正交性难题.根据变分原理，能量泛函的无约束最小化总是倾向于收敛至基态（最小特征值 $\lambda_1$）.为了捕获第 $k$ 个激发态，必须强制当前的近似解与前 $k-1$ 个已求得的特征函数保持正交.
在深度学习框架下，这种正交性通常通过在损失函数中添加罚项来实现.然而，这不仅引入了额外的超参数，还使得优化图景变得更加崎岖，增加了陷入局部极小值或发生模式塌缩的可能性.
\end{itemize}
...
\begin{equation*}
0<\lambda_{k}-\frac{a^{\mu_{1}}(\xi_{k},\xi_{k})}{\langle \xi_{k},\xi_{k}\rangle}\lesssim C(\Omega,V)\varepsilon_1.
\end{equation*}
...
\left\{\begin{aligned}
-\Delta u+V u &=\lambda^{\mu_{1}} u & & \text { in } \Omega, \\
\left(T + \frac{1}{\mu_{1}}\frac{\partial}{\partial n}\right) u &=0 & & \text { on } \partial \Omega.
\end{aligned}\right.
\end{equation}

具体而言，其特征值 $\lambda^{\mu_1}$ 和 Dirichlet 问题的特征值 $\lambda$ 满足如下误差估计：
\begin{equation*}
0<\lambda-\lambda^{\mu_{1}}\lesssim C(\Omega,V)\frac{1}{\mu_{1}}.
\end{equation*}

随后，我们将证明损失函数 $\mathcal{L}_k$ 的极小化项 $\xi_{k}$ 恰为Robin边值问题 \eqref{eq:eigenrobinproblem} 的特征向量.

我们首先给出第一部分的证明.为此，我们定义算子 $A: L^{2}(\Omega) \to L^{2}(\Omega)$ 为 Dirichlet 边值问题的源项到解的映射，即：
\begin{equation}\label{eq:Adef2}
\left\{\begin{aligned}
(-\Delta+V) Af &= f & &\text{in } \Omega, \\
...
\left\{\begin{aligned}
(-\Delta+V) A^{\mu_{1}}f &= f & &\text{in } \Omega, \\
...
文献 \cite{Maury2009Numerical}（命题 2.3 的证明）已确立了 $A^{\mu_{1}}$ 强收敛于 $A$ 这一结论.尽管该结果对于求解椭圆方程已足够，但尚不足以直接证明特征值问题的收敛性.我们需要证明对于一列特征函数序列 $\{\xi_k\}_{k=1}^{\infty}$，都有$A^{\mu_1}\xi_k$在 $H^1(\Omega)$ 中收敛于 $A\xi_k$.因此，需要将进一步证明 $A^{\mu_{1}}$ 在算子范数意义下收敛于 $A$.
...
上式的推导表明，$A^{\mu_{1}}f$ 正是泛函 $R^{\mu_{1}}$ 的极小化项.我们构造如下的测试函数$v = Af - \frac{1}{\mu_1}\frac{\partial Af}{\partial n}$并带入$R^{\mu}(u)$中，我们得到如下估计：
\begin{equation*}
\begin{aligned}
...
\begin{equation*}
\begin{aligned}
\langle A^{\mu_{1}}f, f\rangle - \langle Af, f\rangle &= \langle A^{\mu_{1}}f, (-\Delta+V) Af\rangle - \langle Af, f\rangle \\
&= \langle (-\Delta+V)A^{\mu_{1}}f, Af\rangle - \langle Af, f\rangle \\
...
\begin{equation*}
\langle\delta \mathcal{L}_{k}(u),v\rangle=a^{\mu_1}(u,v)+\mu_2\left[Q(u) \langle v, u\rangle+ \sum\limits_{i<k}\langle v,\xi_{i} \rangle\langle \xi_{i},u \rangle\right].
\end{equation*}

首先考虑 $k = 1$ 的情形.令 $\delta \mathcal{L}_{1}(\xi_1)=0$，可导出 
\begin{equation}\label{eq:3.16}
\left\{
\begin{aligned}
\left[-\Delta+V+\mu_2Q(\xi_{1})\right]\xi_{1}&=0,\\
...
\begin{equation*}
\begin{aligned}
\delta^{2} \mathcal{L}_{k}(u)\langle v,v\rangle=& \frac{1}{2}a^{\mu_{1}}(v,v) +\frac{\mu_{2}}{2}\left[2\langle u, v\rangle^{2}+Q(\xi_{k})\|v\|^{2}+\sum_{i<k}\left\langle\xi_{i}, v\right\rangle^{2}\right].
\end{aligned}
\end{equation*}

若 $l_{1}=1$，即 $\xi_{1}$ 是对应于主特征值 $\lambda_{1}$ 的特征向量，满足 $\|\xi_{1}\|^2=1-\frac{\lambda_{1}}{\mu_{2}}$.此时我们可以证明：
$$\begin{aligned}
\delta^{2} \mathcal{L}_{1}(\xi_1)\langle\mathcal{P}_{>1}v,\mathcal{P}_{>1}v\rangle
&=\frac{1}{2}a^{\mu_{1}}(\mathcal{P}_{>1}v,\mathcal{P}_{>1}v)+\frac{\mu_{2}}{2}\left[2\langle\xi_1, \mathcal{P}_{>1}v\rangle^{2}+Q(\xi_{1})|\mathcal{P}_{>1}v|^{2}\right]\\
&\geq\frac{1}{2}\lambda_{2}|\mathcal{P}_{>1}v|^{2}+\frac{\mu_{2}}{2}\left(-\frac{\lambda_{1}}{\mu_{2}}|\mathcal{P}_{>1}v|^{2}\right)\\
&\geq\frac{1}{2}\delta\lambda_{1}|\mathcal{P}_{>1}v|^{2}>0,
\end{aligned}$$

注意到 $\mathcal{P}_{\leq 1}v$ 满足方程\eqref{eq:3.16}，我们有：
\begin{equation}\label{eq:3.20}
\delta^{2} \mathcal{L}_{1}(\xi_1)\langle\mathcal{P}_{\leq 1}v,\mathcal{P}_{\leq 1}v\rangle=\frac{\mu_2}{2}\langle\xi_1, \mathcal{P}_{\leq1}v\rangle^{2}\geq\frac{1}{2}(\mu_2-\lambda_{1})|\mathcal{P}_{\leq 1}v|^{2}>0.
\end{equation}

这证明了 $\xi_1$ 不仅是临界点，而且是局部极小值点.

反之，若 $l_{1}> 1$，选取 $v_{1}$ 为特征值 $\lambda_{1}$ 对应特征空间中的向量，则有：
$$\begin{aligned}
\delta^{2} \mathcal{L}_{1}(\xi_1)\langle v_{1},v_{1}\rangle&=\frac{1}{2}a^{\mu_{1}}(v_{1},v_{1})+\frac{\mu_{2}}{2}\left[2\langle \xi_{1}, v_{1}\rangle^{2}+Q(\xi_{1})|v_{1}|^{2}\right]\\
&=\frac{\lambda_{1}}{2}|v_{1}|^{2} +\frac{\mu_{2}}{2}\left(-\frac{\lambda_{l_{1}}}{\mu_{2}}|v_{1}|^{2}\right)\\
&=\frac{1}{2}(\lambda_{1}-\lambda_{l_1})|v_{1}|^{2}< 0.
\end{aligned}$$
...
\begin{equation*}
\delta^{2} \mathcal{L}_{1}(\xi_1)\langle v,v\rangle=\frac{1}{2}\left[a^{\mu_{1}}(v,v)-\mu_{2}|v|^{2}\right].
\end{equation*}
...
$$\left\{
\begin{aligned}
\left[-\Delta + V + \mu_{2}Q(\xi_{k})\right]\xi_{k}&=-\mu_{2} \sum\limits_{i<k}\langle \xi_{k},\xi_{i}\rangle\xi_{i},\\
...
\begin{equation*}
[-\Delta + V + \mu_{2}Q(\xi_{k})]\left(\xi_{k}+\sum_{i\in A_{k}}\alpha_{ki}\xi_{i}\right)=-\mu_{2}\sum_{i\in A_{k}^{c}}\langle \xi_{k},\xi_{i}\rangle\xi_{i}.
\end{equation*}

由 Fredholm 二择一原理，方程右端必须正交于核空间，这意味着对 $i\in A_{k}^{c}$ 必有 $\langle \xi_{k},\xi_{i}\rangle=0$.从而方程化为齐次形式：
\begin{equation*}
[-\Delta + V + \mu_{2}Q(\xi_{k})]\left(\xi_{k}+\sum_{A_{k}}\alpha_{ki}\xi_{i}\right)=0.
\end{equation*}
...
由归纳假设，$\{\lambda_{i}\}_{1}^{k-1}$ 对应的特征空间由 $\{\xi_{i}\}_{1}^{k-1}$ 张成，因此必有 $\lambda_{l_{k}}\geq \lambda_{k}$.若假设 $\lambda_{l_{k}}>\lambda_{k}$，则存在一个特征值为 $\lambda_{k}$ 的特征向量 $v$，且 $v$ 与 $\{\xi_{i}\}_{1}^{k}$ 均正交.重复前述推导可得：
$$\delta^{2} \mathcal{L}_{k}(\xi_k)\langle v,v\rangle=\frac{1}{2}(\lambda_{k}-\lambda_{l_k})|v|^{2}<0.$$

因此，$\xi_k$ 是 $\mathcal{L}_{k}$ 的鞍点而非极小值点，矛盾.同 $k=1$ 的情形可证，$\xi_{k}=\vec{0}$ 不成立.故只能有$\lambda_{l_{k}}= \lambda_{k}$，证毕.
\end{proof}

\subsection{数值算法实现}
在众多基于神经网络求解偏微分方程的数值策略中，DRM \cite{Weinan2017The} 建立了一套基于变分原理的求解框架.该方法在执行上可概括为以下四个关键步骤：
\begin{enumerate}
...
下面将 DRM 应用于求解上述变分极小化问题\eqref{eq:conloss}.令假设空间为：
$$\mathcal{N}^{1,2}=\mathcal{N}(\mathcal{D},\mathcal{W},\{2\},m).$$

针对第 $k$ 个特征函数，定义其连续损失函数为：
...
\begin{aligned}
\widehat{\mathcal{L}}_{k}(v) = & \frac{|\Omega|}{N}\sum\limits_{i=1}^{N} \left[ \frac{|\nabla v(X_{i})|^{2}}{2}+\frac{V(X_{i})v^{2}(X_{i})}{2}\right]+\frac{\mu_{1}}{2}\frac{|\partial\Omega|}{M}\sum\limits_{j=1}^{M}\left[ Tv^{2}(Y_{j}) \right] \\
...
为简化表述，下文将统一样本量 $N$ 和 $M$ 为 $n$，算法总结为以下伪代码\ref{alg:DRM_Eigen}.
\par\noindent
\begin{minipage}{\textwidth}
\renewcommand*\footnoterule{}
\renewcommand{\thempfootnote}{\fnsymbol{mpfootnote}}
\begin{algorithm}[H]
\caption{基于 DRM  的多重特征值求解算法}
\label{alg:DRM_Eigen}
\begin{algorithmic}[1]
\Require 计算区域 $\Omega$ 及边界 $\partial\Omega$；需要求解的特征值数量 $k$；由参数 $\phi$ 参数化的神经网络模型 $v_{\phi} \in \mathcal{N}^{2}$；惩罚参数 $\mu_1, \mu_2$；每轮迭代采样点数 $N, M$；最大迭代次数 $T_{max}$ 以及学习率 $\eta$.
\State 初始化空集合保存已求得的特征函数：$\mathcal{V} \leftarrow \emptyset$.
\State \textcolor{white!30!black}{\texttt{\# 外层循环：利用正交惩罚，逐级求解第 $j$ 个特征函数.}}
\For {$j=1:k$}
\State {初始化当前神经网络的参数 $\phi$.}
\State \textcolor{white!30!black}{\texttt{\# 内层循环：训练网络以最小化包含多重惩罚项的经验损失函数.}}
\For {$t=1:T_{max}$}
\State {在区域内部独立采样 $\{X_i\}_{i=1}^N \sim U(\Omega)$，在边界上独立采样 $\{Y_i\}_{i=1}^M \sim U(\partial\Omega)$.}
\State {计算瑞利商与边界惩罚项:
$$J_{base} = \frac{|\Omega|}{N}\sum_{i=1}^{N} \left[ \frac{|\nabla v_{\phi}(X_{i})|^2}{2}+\frac{V(X_{i})v_{\phi}^{2}(X_{i})}{2}\right] + \frac{\mu_{1}}{2}\frac{|\partial\Omega|}{M}\sum_{i=1}^{M} T v_{\phi}^{2}(Y_{i})$$}
\State {计算特征函数归一化约束项:
$$J_{norm} = \frac{\mu_{2}}{4}\left[\frac{|\Omega|}{N}\sum_{i=1}^{N}|v_{\phi}(X_{i})|^{2}-1\right]^{2}$$}
\State {计算正交化约束项（利用集合 $\mathcal{V}$ 中已求得的前 $j-1$ 个特征函数 $\widehat{\xi}_{\phi l}$）:
$$J_{orth} = \frac{\mu_{2}}{2}\sum_{l=1}^{j-1}\left[\frac{|\Omega|}{N}\sum_{i=1}^{N}v_{\phi}(X_{i})\widehat{\xi}_{\phi l}(X_{i})\right]^{2}$$}
\State {计算总的离散经验损失：$\widehat{\mathcal{L}}_{\phi j}(v_{\phi}) = J_{base} + J_{norm} + J_{orth}$.}
\State {反向传播计算梯度 $g_{\phi} = \nabla_{\phi}\widehat{\mathcal{L}}_{\phi j}$.}
\State {通过 Adam 或 SGD 优化器更新 $\phi$：$\phi \leftarrow \phi - \eta g_{\phi}$.}
\EndFor
\State {记录当前收敛的特征函数：$\widehat{\xi}_{\phi j} \leftarrow v_{\phi}$，并更新集合 $\mathcal{V} \leftarrow \mathcal{V} \cup \{\widehat{\xi}_{\phi j}\}$.}
\EndFor
\Ensure {近似多重特征函数序列 $\mathcal{V} = \{\widehat{\xi}_{\phi 1}, \dots, \widehat{\xi}_{\phi k}\}$.}
\end{algorithmic}
\end{algorithm}
\end{minipage}
...
我们的分析主要依赖于椭圆方程的正则性理论、Stein 延拓定理 \cite{fefferman_2005} 以及 Hon 等人 \cite{hon2022simultaneous,yarotsky2017error} 提出的神经网络逼近理论.\cite{hon2022simultaneous} 中的逼近定理如下所述：
\begin{lemma}[连续函数的神经网络逼近\cite{hon2022simultaneous}]\label{th:apperr}
假设 $\tilde{\xi}_{k} \in C^{s}\left([0,1]^{d}\right)$，其中 $s\in \mathbb{N}^{+}$，且对于任意满足 $|\alpha| \leq s$ 的 $\alpha \in \mathbb{N}^{d}$，有 $\left\|\partial^{\alpha} \tilde{\xi}_k\right\|_{\mathcal{D}^{\infty}\left([0,1]^{d}\right)}<1$.对于任意满足 
$$\left(D-2-\log _{2} W\right) W \geq s$$
的 $W,D \in  \mathbb{N}^{+}$，存在一个宽度为 $\mathcal{W}=16 s^{d+1} d(W+2) \log _{2}(8 W)$ 且深度为 $\mathcal{D} = 27 s^{2}(D+2) \log _{2}(4 D)$ 的 $u_{\phi}\in\mathcal{N}^{2}$，使得
\begin{equation*}
...
如前节所述，由于所有特征函数均具备 Holder 连续性，其必然是本质有界的.基于此，本节假设所有相关函数在 $L^{\infty}(\Omega)$ 空间内一致有界，且上界为 $\mathcal{B}$.鉴于 $\mathcal{L}_{k}$ 与 $\mathcal{L}_{\phi k}$ 具有相同的推导逻辑，下文将着重针对 $\mathcal{L}_{k}$ 的误差进行界定.
...
\mathcal{L}_{k,2}(v) &= |\Omega|\mathop{\mathbb{E}}_{X\sim U(\Omega)}\left[ \frac{V(X)v^{2}(X)}{2} \right], \quad &\widehat{\mathcal{L}}_{k,2}(v) &= \frac{|\Omega|}{N}\sum_{i=1}^{N}\left[ \frac{V(X_{i})v^{2}(X_{i})}{2} \right], \\
...
统一记 $\mu$ 为 $\Omega$ 或 $\partial \Omega$ 上的均匀分布.给定 $n$ 个来自 $\mu$ 的独立同分布样本 $\mathbf{Z}_{n}=\{Z_{i}\}_{i=1}^{n}$（其中 $Z_i$ 代表 $X_i$ 或 $Y_i$，且 $n$ 对应 $N$ 或 $M$），我们将用Rademacher 复杂度来量化神经网络假设空间 $\mathcal{N}$ 在给定经验样本 $\mathbf{Z}_{n}$ 上的容量.

界定统计误差的整体逻辑框架如下：1.定义容量指标：在定义 \ref{def:staerr:radcom} 至 \ref{def:staerr:pdim} 中，依次了给出Rademacher复杂度 $\mathfrak{R}$、覆盖数$\mathcal{C}_{\infty}$ 以及伪维数（Pdim）的严谨数学定义.2.误差项分解控制：利用Rademacher复杂度，通过引理 \ref{lemma:staerr:Rad234} 对上述各分解项的泛化间隙进行约束.3.复杂度链式传递：引理 \ref{lemma:staerr:Rad_cover} 建立起通过覆盖数积分控制Rademacher复杂度的桥梁；随后，引理 \ref{lemma:staerr:cover_Pdim} 进一步证明覆盖数可由伪维数所控制.4.网络结构关联：引理 \ref{lemma:staerr:pdim} 显式给出了具有特定深度与宽度的神经网络的伪维数阶数.最终，定理 \ref{thm:staerr:finalthm} 将上述所有理论组件整合，推导出总体统计误差的显式上界.
...
\begin{equation*}
\begin{aligned}
&\mathbb{E}_{\left\{X_{i}\right\}_{i=1}^{N}} \sup _{u \in \mathcal{N}^{2}}\left|\mathcal{L}_{1}(u)-\widehat{\mathcal{L}}_{1}(u)\right| \lesssim \frac{|\Omega|}{2}\mathfrak{N}\left(\mathcal{N}^{1,2}\right),\\
&\mathbb{E}_{\left\{X_{i}\right\}_{i=1}^{N}} \sup _{u \in \mathcal{N}^{2}}\left|\mathcal{L}_{2}(u)-\widehat{\mathcal{L}}_{2}(u)\right| \lesssim \frac{|\Omega|\|V\|_{\infty}\mathcal{B}}{2}\mathfrak{N}\left(\mathcal{N}^{2}\right),\\
...
\begin{lemma}[Lipschitz 收缩不等式]\label{lemma:staerr:liprad}
...
$$ \sup _{u \in \mathcal{N}^{2}}\left|\mathcal{L}_{k, 2}(u)-\widehat{\mathcal{L}}_{k, 2}(u)\right| \leq |\Omega|\mathfrak{R}\left(\left\{V u^{2}:u\in \mathcal{N}^{2}\right\}\right) \leq 2|\Omega|\mathcal{B}^{2}\mathfrak{R}\left(\mathcal{N}^{2}\right). $$
...
\begin{lemma}\label{lem:gradcomplexity}
...
为了利用定义 \ref{def:staerr:pdim} 中定义的覆盖数来界定 Rademacher 复杂度，我们参考 Dudley 的经典结果，即如下定理：
...
$$ \begin{aligned} 
&\quad\; \mathbb{E}_{\left\{\sigma_{i}, Z_{i}\right\}_{i=1}^{n}}\left[\sup\limits_{u \in \mathcal{F}} \frac{1}{n}\left|\sum\limits_{i=1}^{n} \sigma_{i} u\left(Z_{i}\right)\right|\right] \\
&= \mathbb{E}_{\left\{\sigma_{i}, Z_{i}\right\}_{i=1}^{n}}\left[\sup\limits _{u \in \mathcal{F}} \frac{1}{n}\left|\sum\limits_{i=1}^{n} \sigma_{i}\left(u\left(Z_{i}\right)-u_{K}\left(Z_{i}\right)\right) + \sum\limits_{j=1}^{K-1} \sum\limits_{i=1}^{n} \sigma_{i}\left(u_{j+1}\left(Z_{i}\right)-u_{j}\left(Z_{i}\right)\right) + \sum\limits_{i=1}^{n} \sigma_{i} u_{1}\left(Z_{i}\right)\right|\right] \\ 
...
&\quad + \sum\limits_{j=1}^{K-1} \mathbb{E}_{\left\{\sigma_{i}, Z_{i}\right\}_{i=1}^{n}}\left[\sup\limits _{u \in \mathcal{F}} \frac{1}{n}\left|\sum\limits_{i=1}^{n} \sigma_{i}\left(u_{j+1}\left(Z_{i}\right)-u_{j}\left(Z_{i}\right)\right)\right|\right] \\
&\quad + \mathbb{E}_{\left\{\sigma_{i}, Z_{i}\right\}_{i=1}^{n}}\left[\sup\limits _{u \in \mathcal{F}} \frac{1}{n}\left|\sum\limits_{i=1}^{n} \sigma_{i} u_{1}\left(Z_{i}\right)\right|\right]. 
\end{aligned} $$
由于零函数 $0 \in \mathcal{F}$，我们可以无妨设定 $\mathcal{F}_{1}=\{0\}$，从而消除上述不等式中的第三项.对于第一项，应用 Rademacher 变量的性质与 H\"{o}lder 不等式：
\begin{equation*}
\begin{aligned}
...
随后的引理揭示了覆盖数与伪维数 (Pdim) 之间的相互关系.为了寻求分段多项式函数的伪维数的上界，我们参考了 \cite{bartlett2019nearly} 中提出的结论，这些结论适用于我们公式中定义的函数类.
...
设 $\mathcal{N}$ 为满足以下条件的函数族\ref{pf:lemma:staerr:pdim}：
\begin{itemize}
...
基于上述引理，不妨记 $\mathcal{N}^{1,2}$ 表示用于逼近梯度 $\nabla u$ 的 $\mathrm{ReLU}$-$\mathrm{ReLU}^{2}$ 神经网络函数类.结合引理\ref{lem:gradcomplexity}中给出的神经网络导数复杂度界，我们可以直接推导出该假设空间的伪维度上界为：
\begin{equation*}
...
设 $\mathcal{D}$ 和 $\mathcal{W}$ 分别表示神经网络假设空间 $\mathcal{N}$ 的深度和宽度参数，$\mathcal{B}$ 为特征函数系 $\{\xi_{l}\}_{l=1}^{k}$ 及权重函数 $V$ 的 $L^{\infty}$ 一致界.则统计误差满足如下上界：
\begin{equation*}
\mathcal{E}_{\text {sta }}\lesssim C(\Omega,\mathcal{B})(\mu_{1}+\mu_{2})\mathcal{D}^{2}\mathcal{W}\sqrt{\mathcal{D}+\log\mathcal{W}}\sqrt{\frac{\log n}{n}}.
\end{equation*}
\end{theorem}
...
\subsection{总误差估计}\label{subsec_main}
基于前文的理论准备，本节将给出本文的主要误差分析结果.
\begin{theorem}{总误差估计}\label{mainthm}
...
且相应的特征值逼近误差满足：
\begin{equation*}
\displaystyle\mathbb{E}\left|\lambda_{k}-\frac{a^{\mu_{1}}(\widehat{\xi}_{\phi k},\widehat{\xi}_{\phi k})}{\|\widehat{\xi}_{\phi k}\|^{2}}\right|\lesssim C(\Omega,V)\frac{1}{\mu_{1}}+\upsilon^{1/2}.
\end{equation*}
其中，综合误差项 $\upsilon$ 的上界为：
\begin{equation*}
\upsilon\lesssim C(k,\mu_{2},m,d,n,\Omega,V)\left(
\psi(\mathcal{D})^{\frac{4-2m}{d}}\psi(\mathcal{W})^{\frac{4-2m}{d}}
+
\mu_{1}\mathcal{D}^2 \mathcal{W} \sqrt{\mathcal{D}+\log \mathcal{W}}\sqrt{\frac{\log n}{n}}
\right).
\end{equation*}
\end{theorem}
...
&+ C(\Omega,V,\mu_{2})\mu_{1}\mathcal{D}^2 \mathcal{W} \sqrt{\mathcal{D}+\log \mathcal{W}}\sqrt{\frac{\log n}{n}}\\
&+C(k,\mu_{2},m,d,\Omega,V)\sqrt{\mathcal{E}_{\text{app}}+\mathcal{E}_{\text{sta}}}.
\end{aligned}
$$
...
\begin{corollary}
记 $\alpha=\frac{m-2}{d}$.在与定理 \ref{mainthm} 相同的假设条件下，若我们取网络深度、宽度以及惩罚参数满足：
\begin{equation*}
\mathcal{D}=\mathcal{O}(1),\quad\mathcal{W}=\widetilde{\mathcal{O}}(n^{-\frac{1}{6\alpha+2}}),\quad \mu_{1}=\widetilde{\mathcal{O}}(n^{-\frac{\alpha}{6\alpha+2}}),
\end{equation*}
则特征值在期望意义下的收敛速率为$\widetilde{\mathcal{O}}\left(n^{-\frac{\alpha}{6\alpha + 2}}\right)$，当平滑度参数 $m \to \infty$ 时，该收敛速率将渐近趋近于 $\widetilde{\mathcal{O}}(n^{-1/6})$.
\end{corollary}
\begin{proof}
假设定理 \ref{mainthm} 中误差上界的三项具有一致的衰减速率 $\widetilde{\mathcal{O}}(n^{-r})$，并记网络深度、宽度以及惩罚参数增长率为：
$$\mathcal{D}=\mathcal{O}(1),\quad\mathcal{W}=\widetilde{\mathcal{O}}(n^{w}),\quad \mu_{1}=\widetilde{\mathcal{O}}(n^{\beta}),$$
可推导出如下参数关系：
$$-\beta=-\alpha w=\frac{1}{2}\beta+\frac{1}{2}w-\frac{1}{4},$$
求解该方程组，可得统一的衰减指数：
$$r = \frac{\alpha}{6\alpha + 2} = \frac{m-2}{2(3m+d-6)}.$$
进一步地，若定义综合误差项 $\delta=\widetilde{\mathcal{O}}\left(n^{-r}\right) = \widetilde{\mathcal{O}}\left(n^{-\frac{\alpha}{6\alpha + 2}}\right)$，则特征函数的估计误差在期望意义下满足：
\begin{equation*}
\mathbb{E} |\mathcal{P}_{\neq k}\widehat{\xi}_{\phi k}|\lesssim C(k,\mu_{2},m,d,\Omega,V)\delta^{1/2},
\end{equation*}
且相应的特征值估计误差满足：
\begin{equation*}
\mathbb{E}\left|\lambda_{k}-\frac{a^{\mu_{1}}(\widehat{\xi}_{\phi k},\widehat{\xi}_{\phi k})}{|\widehat{\xi}_{\phi k}|^{2}}\right|\lesssim C(k,\mu_{2},m,d,\Omega,V)\delta.
\end{equation*}
特别地，考虑到在适当的边界条件下，目标特征函数通常具备较高的正则性（甚至满足完全光滑条件）.当平滑度参数 $m \to \infty$ 时，相应的指标 $\alpha \to \infty$，上述渐近衰减指数 $r$ 将极限趋近于 $\frac{1}{6}$.这意味着随着特征函数光滑性的提升，综合误差 $\delta$ 的收敛速率将渐近趋近于 $\widetilde{\mathcal{O}}(n^{-1/6})$.
\end{proof}

\begin{remark}
从上述推论可以看出，空间维数 $d$ 仅存在于误差上界的常数项中，而并未出现在样本量 $n$ 的收敛阶（即指数项）的分母主导项中.这意味着，与有限元方法（FEM）等网格依赖型传统数值方法相比，本方法的计算复杂度不会随空间维数呈指数级增长，从而在一定程度上缓解了”维数灾难”.然而需要指出的是，在较高维情形下，上界中的常数项 $C(k,\dots)$的膨胀仍可能对算法的实际收敛表现产生一定影响.
\end{remark}


\section{数值实验}\label{sec_num} 
为了验证我们理论的有效性与正确性，我们开展了数值实验，以在不同条件下逼近特征值及其对应的特征函数.
...
\subsection{二维特征值问题和超参数的影响分析} 
在本节中，我们考虑以下特征值问题.为了便于获取精确解，我们考虑一个变量分离形式的势函数：
\begin{equation}\label{eq:example1}
\left\{\begin{aligned}
-\Delta u + \left(\sum_{i=1}^d V(x_{i})\right)u&=\lambda u & & \text { in } \Omega \\
T u &=0 & & \text { on } \partial \Omega,
\end{aligned}\right.
\end{equation}
其中 $\Omega=[0,1]^d$ 且 $V(t)=\exp(-\pi t)$（指数势）.方程 \eqref{eq:conloss} 是一个变量分离方程，其解在每个维度上都表现出各向同性.可以通过利用有限元方法导出参考解.

在本节中，我们首先考虑几种低维情形，并研究计算误差如何随 $\mu_1$、$\mu_2$、神经网络规模以及样本量变化，并与理论结果相对应.本节中所有实验的默认设置如下：$d=2$，学习率 = 0.001，$\mu_1=1000$，$\mu_2=200$，$\mathcal{W} = 100$，$\mathcal{D} = 4$，epoch $= 10000$，样本量 $= 10000/3000$.我们将以此作为各种对照实验的基准线.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figures1/EigFig2023/under-different-lambda1.png}
    \caption{不同 $\mu_1$ 下的特征值逼近}\label{lambda1}
    \label{fig:lambda1}
\end{figure}

首先，我们选取不同的 $\mu_1$ 值并使用 DRM 进行求解.所得结果展示于图 \ref{fig:lambda1}.定理 \ref{mainthm} 指出，随着 $\mu_1$ 的增加，计算得到的特征值将趋近于真实值，实验结果验证了这一观察结论.

接下来，我们将讨论 $\mu_2$ 对解的影响.我们选取了满足以下条件的 $\mu_2$ 值：小于 $2\lambda_1$，略大于 $2\lambda_1$ 但小于 $2\lambda_2$ 和 $2\lambda_3$，略大于 $2\lambda_2$ 和 $2\lambda_3$，以及显著大于 $2\lambda_2$ 和 $2\lambda_3$.相关结果呈现于图 \ref{fig:lambda2}.

根据定理 \ref{mainthm}，当 $\mu_2$ 大于 $2\lambda_k$ 时，第 $k$ 个特征值是可以被计算出来的.实验结果也验证了这一论述.当 $\mu_2=10<2\lambda_1$ 时，算法无法计算出任何特征值.当 $2\lambda_1<\mu_2=62<2\lambda_2$ 时，第一个特征值能够被较好地计算出来，但无法收敛至第二个特征值.当 $\mu_2>2\lambda_2$ 时，所有对应的特征值均可被准确计算.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{figures1/EigFig2023/under-different-lambda2.png}
    \caption{不同 $\mu_2$ 下的特征值逼近}
    \label{fig:lambda2}
\end{figure}

值得注意的是，当 $\mu_2=62$ 时，第三个特征值似乎已被正确计算，这可能令人费解.为了阐明这一点，我们需要借助近似特征函数的热力图，如图 \ref{fig:heatmap} 所示.众所周知，在二维情形下，第二和第三个特征值在数值上相等，但对应于两个正交的特征函数.通过特征函数的热力图可以观察到，在计算第二个特征值时，由于 $\mu_2$ 取值较小，解退化为了第一个特征函数.然而，在计算第三个特征值时，损失函数包含了两个与第一个特征值相关的正交项.因此，这等价于使用 $2\mu_2$ 计算第二个特征值.这一观察结果与我们的理论分析保持一致.
\begin{figure}[htbp]
    \centering
...
\begin{figure}[htbp]
    ...
    \caption{$\mu_2=125$ 和 $\mu_2=200$ 时特征函数逼近的热力图}\label{fig:heatmap}
\end{figure}
...
在本节及下一节中，我们在式\eqref{eq:conloss} 中选取了不同的势函数 $V(x)$，包括常用的函数，如指数函数 $(w(x)=\exp(-\pi x))$ 和平方函数 $(w(x)=x^2)$.此外，我们探索了不同的区域，包括 10 维立方体和 10 维球体.在这些区域上，依赖于空间离散化的传统数值算法（如有限元法和有限差分法）一直无法提供令人满意的结果.实验的参数设置如下：
...
$\mu_1/\mu_2$& 2000/1000&  2000/500& 2000/600\\
宽度/深度
& 150/4 & 150/4 & 60/4 \\
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.48\linewidth}
        ...
        \caption{方形势下的基态特征函数 $\widehat{\xi}_{\phi 1}$}
        \label{fig:sq1}
    \end{subfigure}
    ...
    \begin{subfigure}{0.48\linewidth}
        ...
        \caption{指数势下的基态特征函数 $\widehat{\xi}_{\phi 1}$}
        \label{fig:exp1}
    \end{subfigure}
    ...
    \begin{subfigure}{0.48\linewidth}
        ...
        \caption{方形势下的第二特征函数 $\widehat{\xi}_{\phi 2}$}
        \label{fig:sq2}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\linewidth}
        ...
        \caption{指数势下的第二特征函数 $\widehat{\xi}_{\phi 2}$}
        \label{fig:exp2}
    \end{subfigure}
    ...
    \begin{subfigure}{0.48\linewidth}
        ...
        \caption{方形势下的第三特征函数 $\widehat{\xi}_{\phi 3}$}
        \label{fig:sq3}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\linewidth}
        ...
        \caption{指数势下的第三特征函数 $\widehat{\xi}_{\phi 3}$}
        \label{fig:exp3}
    \end{subfigure}
...
如图 \ref{fig1} 和 \ref{fig2} 所示，所有估计的特征值和特征函数均与其各自的精确值非常接近.计算得到的特征值小于精确值，这与定理 \ref{mainthm} 的结论一致.
...
对于 $\mathbb{R}^{10}$，结果展示在图 \ref{fig3} 中，在此设置下，所提出的方法较好地估计了特征值和特征函数.

对于 $\mathbb{R}^{20}$ 的情形，使用蒙特卡洛方法计算积分面临一定的挑战.具体而言，高维空间区域中的均匀采样会导致采样点在边界附近相对集中.为了应对这一挑战，常见的策略包括增加采样点数量以及实施更高效的采样技术.作为一种解决方案，我们采用了分层采样方法.该方法将单位球划分为两个不同的区域，记为 $B_{20}(0,1/2)$ 和 $B_{20}(0,1)/B_{20}(0,1/2)$.我们对每个区域分别计算蒙特卡洛积分，随后将所得数值进行加权求和.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{20维单位球上的基态特征函数 $\widehat{\xi}_{\phi 1}$}
        \label{fig:xi_rank1}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{20维单位球上的第一特征值 $\lambda_{1}$}
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{20维单位球上的第二特征函数 $\widehat{\xi}_{\phi 2}$}
        \label{fig:xi_rank2}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{20维单位球上的第二特征值 $\lambda_{2}$}
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{20维单位球上的第三特征函数 $\widehat{\xi}_{\phi 3}$}
        \label{fig:xi_rank3}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{20维单位球上的第三特征值 $\lambda_{3}$}
...
本文研究了利用神经网络求解椭圆多重特征值问题.我们基于椭圆特征值问题的惩罚变分形式，提出了一种计算椭圆多重特征值的通用形式.我们利用 DRM 求解了该惩罚变分形式.我们建立了估计特征值与真实特征值之间误差的上界，该上界与网络的深度 $\mathcal{D}$、宽度 $\mathcal{W}$ 以及训练样本量 $n$ 有关.通过探讨特征值问题的正则性并适当选择深度 $\mathcal{D}$ 和宽度 $\mathcal{W}$，我们表明该误差界在一定程度上缓解了维数灾难的影响.若干模拟结果验证了所提方法的有效性，并支持了我们的理论结果.

% \section{本章附录}\label{app:proofingen}
