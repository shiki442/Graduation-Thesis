...
\item[(b)] 内部源项满足 $g \in L^2(\Omega)$，边界函数满足 $h \in H^{1/2}(\partial \Omega)$。
\item[(c)] 非线性项 $V(u)$ 的增长速度满足 Sobolev 次临界或临界增长条件。即存在常数 $C > 0$，使得对于任意 $s \in \mathbb{R}$，有：
$$|V(s)| \le C(1 + |s|^p)$$
其中，当空间维度 $N \ge 3$ 时，指数 $p \le \frac{N+2}{N-2}$；当 $N=1, 2$ 时，$p \in [1, +\infty)$ 可以为任意有限值。
...
\end{enumerate}
\end{assumption}
值得强调的是，假设 \ref{ass:f}(d) 中对 $V(u)$ 的限制主要是为了确保能量泛函的严格凸性。若仅考察弱解的存在性，该条件可被适当弱化。
...
则有以下结论：
\begin{itemize}
\item 对于有限的惩罚参数 $\mu<+\infty$，$J^{\mu}$ 存在唯一的极小值点，记为 $u^{\mu}$。且该极小值点是方程 \eqref{eq:maineq} 的弱解。
\item 当 $\mu \rightarrow +\infty$ 时，设 $u^{D}$ 为具有精确 Dirichlet 边界条件（即在 $\partial \Omega$ 上 $u=h$）的方程 \eqref{eq:maineq} 的解。则存在仅依赖于区域和已知函数的常数 $C(\Omega,V,g,h)$，使得：
\begin{equation*}
...
\end{equation*}
最后，由于惩罚参数 $\mu > 0$，根据广义 Poincaré 不等式，存在仅依赖于区域 $\Omega$ 和 $\mu$ 的常数 $C(\Omega,\mu)>0$，使得上式右侧以 $C(\Omega,\mu)\|v\|_{H^{1}}^{2}$ 为下界。证毕。
\end{proof}
...
\leq&\displaystyle\underbrace{ \inf _{u_{\text{nn}} \in \mathcal{N}^{2}}\left[ J^{\mu}(u_{\text{nn}})- J^{\mu}(u^{*})\right]}_{\mathcal{E}_{\text{app}}}+\underbrace{2 \sup _{u_{\text{nn}} \in \mathcal{N}^{2}}\left|J^{\mu}_{K}(u_{\text{nn}})-\widehat{J}^{\mu}_{K}(u_{\text{nn}})\right|}_{\mathcal{E}_{\text{sta}}}+\underbrace{C(g)\mathcal{B}K^{1-p}}_{\mathcal{E}_{\text{tru}}},&\\
\end{aligned}
\end{equation*}
...
\begin{equation*}
|J_{K}^{\mu}(u_{\text{nn}})-J^{\mu}(u_{\text{nn}})|\leq\int_{\Omega}|u_{\text{nn}}(g-g^K)|\,dx\leq C(g)\mathcal{B}K^{1-p}.
\end{equation*}
...
\begin{equation*}
\mathcal{E}_{a p p}=\inf _{u_{\text{nn}} \in \mathcal{N}^{2}(\mathcal{D},\mathcal{W},\mathcal{B})}\left[J^{\mu}\left(u_{\text{nn}}\right)-J^{\mu}\left(u^{*}\right)\right]\leq C(\Omega,u^{*},f)\mu\left(\mathcal{B}^{-2/(d-2)}+\mathcal{W}^{-1/d}\right).
\end{equation*}
\end{theorem}
\begin{proof}
...
&\leq C(\Omega,f)\mu\inf\limits _{u_{\text{nn}} \in \mathcal{N}^{2}}\|u_{\text{nn}}-u^{*}\|_{H^{1}}^{2}\\
&\leq C(\Omega,f)\mu\left(\inf\limits _{u_{\text{nn}} \in \mathcal{N}^{2}}\|u_{\text{nn}}-u^{*}_{\mathcal{B}}\|_{H^{1}}^{2}+\|u^{*}_{\mathcal{B}}-u^{*}\|_{H^{1}}^{2}\right)\\
&\leq C(\Omega,f,u^{*})\mu\left(\mathcal{W}^{-1/d}+\mathcal{B}^{-q/d}\right).
\end{aligned}
\end{equation*}
\end{proof}
...
具体而言，各项定义如下：
$$ \begin{aligned} 
\mathcal{L}_{1}^{\varepsilon}(v)&=|\Omega|\mathop{\mathbb{E}}_{X\sim U(\Omega)}\left[\|\nabla v(X)\|_{2}^{2}\right], &\widehat{\mathcal{L}}_{1}^{\varepsilon}(v)&=\frac{|\Omega|}{N}\sum\limits_{i=1}^{N}\left[ \|\nabla v(X_{i})\|_{2}^{2}\right], \\ 
\mathcal{L}_{2}^{\varepsilon}(v)&=2|\Omega|\mathop{\mathbb{E}}_{X\sim U(\Omega)}\left[F(v(X))\right], &\widehat{\mathcal{L}}_{2}^{\varepsilon}(v)&=2\frac{|\Omega|}{N}\sum\limits_{i=1}^{N}\left[F(v(X_{i}))\right], \\
\mathcal{L}_{3}^{\varepsilon}(v)&=-2|\Omega|\mathop{\mathbb{E}}_{X\sim U(\Omega)}\left[v(X)g_{K}(X)\right], &\widehat{\mathcal{L}}_{3}^{\varepsilon}(v)&=-2\frac{|\Omega|}{N}\sum\limits_{i=1}^{N}\left[v(X_{i})g_{K}(X_{i})\right], \\
\mathcal{L}_{4}^{\varepsilon}(v)&=\varepsilon |\partial \Omega|\mathop{\mathbb{E}}_{Y\sim U(\partial\Omega)}\left[ v^{2}(Y) \right], &\widehat{\mathcal{L}}_{4}^{\varepsilon}(v)&=\varepsilon \frac{|\partial\Omega|}{M}\sum\limits_{j=1}^{M}\left[v^{2}(Y_{j}) \right],\\
\mathcal{L}_{5}^{\varepsilon}(v)&=-2\varepsilon |\partial \Omega|\mathop{\mathbb{E}}_{Y\sim U(\partial\Omega)}\left[ v(Y)h(Y) \right], &\widehat{\mathcal{L}}_{5}^{\varepsilon}(v)&=-2\varepsilon \frac{|\partial\Omega|}{M}\sum\limits_{j=1}^{M}\left[v(Y_{j})h(Y_{j}) \right]. 
\end{aligned} $$


这里，$U(\Omega)$ 和 $U(\partial\Omega)$ 分别表示在 $\Omega$ 和 $\partial\Omega$ 上的均匀分布。给定来自均匀分布的 $n$ 个独立同分布 (i.i.d) 样本 $\mathbf{Z}_{n}=\left\{Z_{i}\right\}_{i=1}^{n}$，我们可以利用\\ Rademacher 复杂度来评估给定函数类 $\mathcal{N}$ 在 $n$ 个随机样本 $\mathbf{Z}_{n}$ 上的限制容量。
...
\begin{theorem}\label{thm:staerr:ch3finalthm}
\begin{equation*}
...
\subsection{主要定理}\label{sec:mainres}
针对 Robin 边界条件，我们有：
...
\begin{proof}
由引理 \ref{th:energyerr} 以及第 \ref{sec:apperr} 节中逼近误差和第 \ref{sec:staerr} 节中统计误差的估计，我们有：
\begin{equation*}
...
首先，引入标准的磨光核（mollifier）$\rho\in C_{0}^{\infty}(\mathbb{R}^d)$，满足如下性质：$\int \rho\,dx=1$ 且 $\rho \geq 0$；当 $\|x\|> 1$ 时 $\rho(x) = 0$；$\|\rho(x)\|_{\infty}=2V_{d}$，其中 $V_{d}$ 为 $d$ 维单位球的体积。取 $q = \frac{2d}{d-2}$，定义卷积平滑后的函数为：
\begin{equation*}
