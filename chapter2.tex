...
更为关键的是，当前绝大多数深度神经求解算法严重缺乏系统的理论支撑。与具备完备误差分析体系的传统线性泛函分析方法不同\cite{duan2022convergence,mishra2022estimates,lu2021deepxde}，神经网络求解本质上是一个高度非线性的机器学习过程。大量采样数据的使用引入了传统逼近理论中罕见的统计泛化误差；同时，神经网络的函数空间难以直接纳入经典的索伯列夫（Sobolev）或赫尔德（Hölder）等基于正则性的框架中。尽管Lu等人\cite{lu2021priorieigen}在谱Barron空间（Spectral Barron space）内，针对高维薛定谔问题的两层神经网络给出了先验泛化误差分析，初步论证了神经网络克服维数灾难的潜力，但该空间的严苛定义限制了其在更广泛物理情形下的适用性。此外，现有理论大多局限于系统主特征值（基态）的求解。根据极大极小原理（Min-Max Principle），系统的第 $K$ 个特征值在变分意义上对应于能量泛函的 $K$ 阶鞍点（Saddle point）。这种非全局极小的几何特性，引出了一个极具挑战性的核心问题：如何构造一个既稳定又实用的变分形式，以实现多重特征值（激发态）的同步高精度求解？
