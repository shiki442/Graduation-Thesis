...
\begin{enumerate}
    \item \textbf{实谱性}：所有特征值 $\lambda$ 均为实数。
    \item \textbf{离散性与排序}：特征值构成一个可列的离散集合，且仅以 $+\infty$ 为聚点。若考虑代数重数，特征值可按非递减顺序排列为：
    ...
    \end{equation}
    \item \textbf{完备正交性}：对应的特征函数系 $\{\xi_{k}\}_{k=1}^{\infty}$ 构成 $L^{2}(\Omega)$ 空间的一组完备正交基。
\end{enumerate}
\end{lemma}
...
\quad
\begin{enumerate}
    \item \textbf{内积与范数}：记 $\langle\cdot,\cdot\rangle$ 为 $L^{2}(\Omega)$ 空间的标准内积，$\|\cdot\|$ 为对应的 $L^{2}$ 范数。
    ...
    \item \textbf{渐近符号}：在不等式估计中，使用符号 $A \lesssim B$ 表示存在一个与关键参数无关的有界常数 $C$，使得 $A \leq CB$。若常数依赖于特定参数，将特别注明。
...
函数 $\sigma_{\ell}$ 被称为激活函数。在本文中，我们主要采用$k$次幂整流线性单元（Rectified Power Unit, RePU）作为激活函数，即 $\sigma(x) = \mathrm{ReLU}^{k}(x) = \max\{0, x\}^k$，其中 $k \in \mathbb{N}^{+}$。
...
\begin{enumerate}
    \item \textbf{规模约束}：网络实际深度 $D$ 和宽度 $W$ 满足 $D < \mathcal{D}$ 且 $W < \mathcal{W}$（其中 $D, W \in \mathbb{N}^+$）；
    \item \textbf{激活函数}：所有激活函数均属于族类 $\{\mathrm{ReLU}^{k} \mid k \in \{k_{i}\}\}$；
    \item \textbf{复杂度下界}：网络的结构参数满足如下不等式约束：
    \begin{equation}
...
下面将 Deep Ritz Method (DRM) 应用于求解上述变分极小化问题\eqref{eq:conloss}。令假设空间为 $\mathcal{N}^{2}=\mathcal{N}(\mathcal{D},\mathcal{W},\{2\},m)$。针对第 $k$ 个特征函数，定义其连续损失函数为：
...
%         \State \textbf{构建经验损失函数 $\widehat{\mathcal{L}}_{\phi j}$：}
...
& + \underbrace{2 \sup_{u \in \mathcal{N}^{2}}\mathbb{E}\left|\mathcal{L}_{k}(u)-\widehat{\mathcal{L}}_{k}(u)\right|+2 \sup_{u \in \mathcal{N}^{2}}\mathbb{E}\left|\mathcal{L}_{\phi k}(u)-\widehat{\mathcal{L}}_{\phi k}(u)\right|}_{\mathcal{E}_{\text{sta}}},
\end{aligned}
\end{equation*}
...
我们的分析主要依赖于椭圆方程的正则性理论、Stein 延拓定理 \cite{fefferman_2005} 以及 Hon 等人 \cite{hon2021simultaneous} 提出的神经网络逼近理论。
...
直接引用 \cite{hon2021simultaneous} 中的逼近定理
\cite{hon2021simultaneous} 中的逼近定理如下所述：
...
设集合 $A \subseteq \mathbb{R}^{n}$，该\textbf{集合}经验拉德马赫复杂度定义为：
...
进一步地，给定包含 $n$ 个独立同分布样本的随机向量 $\mathbf{Z}_{n}=(Z_1, \dots, Z_n)$，\textbf{函数类} $\mathcal{N}$ 关于该样本的经验拉德马赫复杂度定义为：
...
记 $\mathcal{N}^{1,2}=\mathcal{N}(\mathcal{D}+3,(\mathcal{D}+2)\mathcal{W},\{1,2\},\infty)$，我们有
\begin{equation*}
\begin{aligned}
...
基于上述引理，并结合式\eqref{eq:N12} 中定义的网络结构，我们可直接推导出特定假设空间 $\mathcal{N}^{1,2}$ 的伪维度界：
\begin{equation*}
\operatorname{Pdim}(\mathcal{N}^{1,2})\lesssim C_2 d^2\mathcal{D}^4\mathcal{W}^2(\mathcal{D}+\log\mathcal{W}).
\end{equation*}
其中，在网络宽度满足 $\mathcal{W}>d$ 的前提下，$C_1$ 与 $C_2$ 均为不依赖于输入维度 $d$ 的绝对常数
...
对于 Rademacher 复杂度 $\mathfrak{N}\left(\mathcal{N}\right)$，利用 Dudley 熵积分不等式，我们有：
$$\begin{aligned}
...
\subsection{核心误差估计}\label{subsec_main}\begin{theorem}{}\label{mainthm}
...
\begin{tabular}{l|ccc}
\toprule
\textbf{参数} & \textbf{平方势} & \multicolumn{1}{p{2.2cm}}{\textbf{指数势}} & \multicolumn{1}{p{3.2cm}}{\textbf{简谐振子}}\\
\midrule
...
\begin{proof}{引理 \ref{lemma:staerr:Rad234} 的证明}\label{pf:lemma:staerr:Rad234}
...
然后我们再来单独处理第一个不等式，由于梯度算子 $\nabla$ 不具备 Lipschitz 连续性，我们无法直接应用引理 \ref{lemma:staerr:liprad}。我们需要证明 $\|\nabla u\|_2^2$ 可以由一个 $\mathrm{ReLU}$-$\mathrm{ReLU}^{2}$ 网络实现。具体而言，我们有如下断言：

\par\noindent{\textbf{断言}：令 $u$ 为由深度 $\mathcal{D}$、宽度 $\mathcal{W}$ 的 $\mathrm{ReLU}^{2}$ 网络实现的函数。那么其梯度平方项 $\|\nabla u\|_2^2$ 可由一个深度为 $\mathcal{D}+3$ 且宽度为 $d\left(\mathcal{D}+2\right)\mathcal{W}$ 的混合网络 $\mathrm{ReLU}$-$\mathrm{ReLU}^{2}$ 来实现。}

为简便起见，下文将 $\mathrm{ReLU}$ 和 $\mathrm{ReLU}^{2}$ 激活函数分别记为 $\sigma_1$ 和 $\sigma_2$。
由于 $\|\nabla u\|_2^2=\sum_{i=1}^{d}\left|D_i u\right|^2$，且平方函数可通过 $x^2=\sigma_2(x)+\sigma_2(-x)$ 精确表示，因此只要我们能证明每个偏导数 $D_iu \ (i=1,2,\cdots,d)$ 均可分别由 $\mathrm{ReLU}$-$\mathrm{ReLU}^{2}$ 网络实现，便能轻易构造出目标网络。

现证明对于任意 $i=1,2,\cdots,d$，$D_iu$ 均可由 $\mathrm{ReLU}$-$\mathrm{ReLU}^{2}$ 网络实现。由于前两层的结构存在微小差异，我们首先对其进行单独分析，随后对 $k\geq3$ 的层运用数学归纳法。对于第一层，注意到 $\sigma_2^{'}(x)=2\sigma_1(x)$，对任意 $q=1,2\cdots,n_1$，有：
\begin{equation*}
...
\end{equation*}
因此，$D_iu_q^{(1)}$ 可由深度为 $2$、宽度为 $1$ 的 $\mathrm{ReLU}$-$\mathrm{ReLU}^{2}$ 子网络实现。对于第二层，计算可得：
\begin{equation*}
...
由于等式右侧的 $\sigma_1(\cdot)$ 项与求和项 $\sum a_{qj}^{(2)}D_iu_j^{(1)}$ 可分别由独立的 $\mathrm{ReLU}-\mathrm{ReLU}^{2}$ 子网络实现，且两个标量的乘法运算亦可通过下式利用神经网络精确拟合：
\begin{equation*}
\begin{split}
...
由此可知 $D_iu_q^{(2)}$ 可由 $\mathrm{ReLU}$-$\mathrm{ReLU}^{2}$ 网络实现。具体而言，相关子网络的复杂度满足：
$$\mathcal{D}\left(\sigma_1\left(\sum_{j=1}^{n_1}a_{qj}^{(2)}u_j^{(1)}+b_q^{(2)}\right)\right)=3, \mathcal{W}\left(\sigma_1\left(\sum_{j=1}^{n_1}a_{qj}^{(2)}u_j^{(1)}+b_q^{(2)}\right)\right)\leq\mathcal{W}$$
以及
$$\mathcal{D}\left(\sum_{j=1}^{n_1}a_{qj}^{(2)}D_iu_j^{(1)}\right)=2, \mathcal{W}\left(\sum_{j=1}^{n_1}a_{qj}^{(2)}D_iu_j^{(1)}\right)\leq\mathcal{W}.$$
综合可得 $\mathcal{D}\left(D_iu_q^{(2)}\right)=4$，且 $\mathcal{W}\left(D_iu_q^{(2)}\right)\leq\max\{2\mathcal{W},4\}$。

接下来，对 $k\geq3$ 时的层结构应用归纳法。对于第三层，有：
\begin{equation*}
...
且
\begin{equation*}
\mathcal{D}\left(\sum_{j=1}^{n_2}a_{qj}^{(3)}D_iu_j^{(2)}\right)=4, \mathcal{W}\left(\sum_{j=1}^{n_2}a_{qj}^{(3)}D_iu_j^{(2)}\right)\leq\max{2\mathcal{W},4\mathcal{W}}=4\mathcal{W},
\end{equation*}
故推得 $D_iu_q^{(3)}$ 亦可由该类网络实现，其复杂度为 $\mathcal{D}\left(D_iu_q^{(3)}\right)=5$ 且 $\mathcal{W}\left(D_iu_q^{(3)}\right)\leq\max\{5\mathcal{W},4\}=5\mathcal{W}$。

归纳假设当层数为 $k$ 时，$D_iu_q^{(k)}$ 能够被实现，且满足 $\mathcal{D}\left(D_iu_q^{(k)}\right)=k+2$, $\mathcal{W}\left(D_iu_q^{(k)}\right)\leq(k+2)\mathcal{W}$。对于第 $k+1$ 层，我们有：
$$ \begin{aligned} D_iu_q^{(k+1)} &= D_i\sigma_2\left(\sum_{j=1}^{n_k}a_{qj}^{(k+1)}u_j^{(k)}+b_q^{(k+1)}\right)\\ &= 2\sigma_1\left(\sum_{j=1}^{n_k}a_{qj}^{(k+1)}u_j^{(k)}+b_q^{(k+1)}\right)\cdot\sum_{j=1}^{n_k}a_{qj}^{(k+1)}D_iu_j^{(k)}. \end{aligned} $$
经网络叠加复杂度计算可知：
\begin{equation*}
\begin{aligned}
...
\begin{equation*}
\begin{aligned}
\mathcal{D}\left(\sum_{j=1}^{n_k}a_{qj}^{(k+1)}D_iu_j^{(k)}\right)&=k+2, \\
\mathcal{W}\left(\sum_{j=1}^{n_k}a_{qj}^{(k+1)}D_iu_j^{(k)}\right)&\leq\max{(k+2)\mathcal{W},4\mathcal{W}}=(k+2)\mathcal{W},
\end{aligned}
\end{equation*}
因此，$D_iu_q^{(k+1)}$ 能够成功实现，且其深度与宽度分别受界于 $\mathcal{D}\left(D_iu_q^{(k+1)}\right)=k+3$ 与 $\mathcal{W}\left(D_iu_q^{(k+1)}\right)\leq\max\{(k+3)\mathcal{W},4\}=(k+3)\mathcal{W}$。
根据归纳原理，总偏导数 $D_iu=D_iu_1^{(\mathcal{D})}$ 可由 $\mathrm{ReLU}$-$\mathrm{ReLU}^{2}$ 网络完整表达，且满足 $\mathcal{D}\left(D_iu\right)=\mathcal{D}+2$ 与 $\mathcal{W}\left(D_iu\right)\leq\left(\mathcal{D}+2\right)\mathcal{W}$。综上，我们得到关于梯度范数平方的网络复杂度界为：
...
\begin{proof}{引理 \ref{lemma:staerr:Rad_cover} 的证明} \label{pf:lemma:staerr:Rad_cover}
...
\begin{proof}[引理 \ref{lemma:staerr:pdim} 的证明]\label{pf:lemma:staerr:pdim}
本引理的证明思路如下：首先，引入 VC 维（VC-dimension）与打散（Shattering）的概念作为估计伪维度（Pdim）的基础；其次，借助文献 \cite{anthony2009neural} 中的结论界定多项式函数族的 VC 维；最后，基于上述结果，采用类似于文献 \cite{bartlett2019nearly} 中定理 6 的参数空间划分技术完成证明。
为行文严谨，我们首先回顾打散与 VC 维的严格定义。
\begin{definition}
...
\begin{definition}[VC 维]\label{def:staerr:vc_dim}
