...
-\Delta u = \lambda u
\end{equation}
此处，特征值 $\lambda = (\omega/c)^2$ 直接关联系统的振动频率，而特征函数 $u$ 则刻画了振动的本征模态。
...
本质上即为带有势能项 $V$ 的椭圆型特征值问题。式中，特征值 $E$ 对应系统容许的量子化能级，特征函数 $\psi$ 则描述粒子在空间出现的概率幅。有界区域上椭圆算子谱的离散性，正是量子力学中能量量子化现象的数学根源。此外，在微分几何领域，估计庞加莱常数或斯捷克洛夫常数等关键几何量，同样高度依赖于高维特征值问题的求解\cite{johnson2004convergence,courtade2020bounds}。


然而，当这些物理模型向多粒子系统或复杂高维空间拓展时（例如多电子系统的薛定谔方程\cite{berezin2012schrodinger,li1983schrodinger,fortunato1998eigenvalue}），传统数值解法遭遇了严峻挑战。诸如有限元法（FEM）和有限体积法（FVM）等经典离散方法，深受“维数灾难”（Curse of Dimensionality）的困扰，其计算成本随空间维数呈指数级增长，导致其在高维环境中几乎失效。
近年来，深度神经网络（DNN）因其强大的高维函数逼近能力，逐渐成为克服维数灾难、求解高维偏微分方程\cite{chen2021friedrichs,Weinan2017The,gu2021selectnet,raissi2019physics,Justin2018DGM,wan2020zang,han2018solving,raissi2017physics,jagtap2021extended}及特征值问题\cite{PhysRevResearc,2019Deep}的新范式。围绕高维特征值问题的深度学习求解，现有研究主要沿着两条主线展开：

第一类方法侧重于利用特定数学结构，或将特征值直接作为网络的可训练参数进行隐式优化。 例如，Han等人\cite{HAN2020109792,HAN2019108929}基于深度BSDE框架提出了类扩散蒙特卡洛方法，将高维特征值问题巧妙地转化为偏微分方程半群算子的不动点问题；随后，Li等人\cite{li2022semigroup}引入约束条件发展了神经网络半群方法，在10维椭圆方程中实现了超越DMC的计算效率。针对特定物理场景，Cai等人\cite{cai2023deepmartnet}设计了处理系数不连续界面的神经网络以捕捉物理跳跃条件；Pfau及Hermann等人则分别针对多电子薛定谔方程，设计了严格满足费米-狄拉克反对称统计规律的深度网络架构（如PauliNet）。尽管这些方法在特定场景下取得了显著成功，但它们普遍存在一定的局限性：过度依赖问题的特殊构造导致算法通用性不足；而将特征值本身作为参数参与损失函数优化，往往会改变原问题的物理变分结构，极易导致优化图景高度非凸，带来训练过程的不稳定，也为后续的严谨数学分析带来了巨大阻碍。

第二类方法则回归特征值的数学本源，即基于瑞利商（Rayleigh Quotient）公式进行无监督学习。 瑞利商不仅赋予了特征值清晰的能量最小化物理直觉，也为神经网络提供了一个极其自然的损失函数格式。Ben-Shaul\cite{ben2023deep}率先提出了基于瑞利商的网络训练框架；Zhang等人\cite{zhang2024deep}在此基础上引入正交性约束，确保了生成的特征函数能够自动满足正交性要求；Yoo等人\cite{yoo2025physics}则进一步将该框架拓展至土木与机械工程领域的结构振动计算中。然而，现有基于瑞利商的方法大多局限于1维或2维等低维简单区域的数值实验，在面对高维复杂物理场景（如高维薛定谔方程）时，如何高效实现高维空间中的正交化约束并保证收敛，仍是一个亟待解决的难题。

更为关键的是，当前绝大多数深度神经求解算法严重缺乏系统的理论支撑。与具备完备误差分析体系的传统线性泛函分析方法不同\cite{duan2022convergence,mishra2023estimates,lu2021deepxde}，神经网络求解本质上是一个高度非线性的机器学习过程。大量采样数据的使用引入了传统逼近理论中罕见的统计泛化误差；同时，神经网络的函数空间难以直接纳入经典的索伯列夫赫尔德等基于正则性的框架中。尽管Lu等人\cite{lu2021priorieigen}在谱Barron空间（Spectral Barron space）内，针对高维薛定谔问题的两层神经网络给出了先验泛化误差分析，初步论证了神经网络克服维数灾难的潜力，但该空间的严苛定义限制了其在更广泛物理情形下的适用性。此外，现有理论大多局限于系统主特征值（基态）的求解。根据极大极小原理，系统的第 $K$ 个特征值在变分意义上对应于能量泛函的 $K$ 阶鞍点。这种非全局极小的几何特性，引出了一个极具挑战性的核心问题：如何构造一个既稳定又实用的变分形式，以实现多重特征值（激发态）的同步高精度求解？

针对上述算法通用性与理论基础的双重缺失，本文旨在开发一种系统的方法，以求解高维空间中的多重椭圆型特征值（Elliptic Multiple Eigenvalue）问题。本研究不仅致力于获取高精度的多阶特征值与特征函数，更着重于提供一套独立于空间维数的严密先验误差分析理论。具体而言，本文首先基于椭圆型特征值问题的惩罚变分原理，提出了一种计算多重EME的通用无约束优化公式。随后，我们采用深度里兹方法对该惩罚变分形式进行高效求解。在理论层面，我们建立了一个刻画计算特征值与真实特征值之间误差的严格上界，该界限由前馈神经网络的深度 $\mathcal{D}$、宽度 $\mathcal{W}$ 以及训练样本量 $n$ 共同显式表达。通过深入挖掘EME特征函数的内在正则性，并自适应地配置网络容量参数 $\mathcal{D}$ 和 $\mathcal{W}$，我们从理论上严谨地证明了该方法可达到 $O(n^{-1/16})$ 的收敛率。这一结论不仅填补了多重特征值神经网络求解的理论空白，更在数学上严格证明了本方法能够有效规避维数灾难。

此外，本文针对所提出的惩罚变分格式与深度里兹求解框架，精心设计了一系列具有代表性的高维数值实验。本文在最高达20维的复杂算例上开展了实验验证。在如此高的空间维度下，传统数值方法早已彻底失效，而本文提出的深度神经求解器所计算出的前几阶特征值的相对误差仍能保持在 $10^{-2}$ 量级，并且能够准确捕捉相应的本征模态特征。这一数值表现不仅彰显了本文方法在处理超高维特征值问题上的强大能力，更为前文所推导的先验误差分析与收敛率理论提供了强有力的实证支撑。

本文的组织结构如下：第\ref{sec_pre}节概述全文所需的基本假设、预备知识与符号系统；第\ref{sec_model}节详细推导多重EME的惩罚变分公式及神经网络求解框架；第\ref{sec_main}节提供核心的先验误差分析与收敛率证明；第\ref{sec_num}节通过数值实验验证算法在高维场景下的有效性与理论分析的客观性；最后，第\ref{sec dis}节总结全文，相关定理的严谨证明则完整收录于附录\ref{app:proofingen}中。
...
特征值问题的正则性是本文分析的另一个核心要素。足够高的正则性对于避免高维问题中的“维数灾难”至关重要（详见第 \ref{sec_main} 节）。为此，我们提出如下假设：
...
针对一般区域的 PDE 问题，边界惩罚法提供了一种更为灵活的“软约束”策略 \cite{duan2021analysis, Mller2021ErrorEF}。其核心思想是引入如下修正的双线性形式：
...
更为严重的是，这种非凸性导致优化过程在原点附近极不稳定。当近似解的模长趋于零（$\|v\| \to 0$）时，瑞利商的梯度计算将出现奇异性，导致优化问题变得病态。为克服这一平凡解风险，必须在损失函数中引入适当的正则化项或稳定项\cite{Weinan2017The}，以约束解的模长并维持数值稳定性。
...
\subsection{数值算法实现}
在众多基于神经网络求解偏微分方程（PDE）的数值策略中，鄂维南等人提出的 Deep Ritz 方法\cite{Weinan2017The} 建立了一套基于变分原理的求解框架。该方法在执行上可概括为以下四个关键步骤：
\begin{enumerate}
...
下面将 Deep Ritz 法应用于求解上述变分极小化问题\eqref{eq:conloss}。令假设空间为 $\mathcal{N}^{1,2}=\mathcal{N}(\mathcal{D},\mathcal{W},\{2\},m)$。针对第 $k$ 个特征函数，定义其连续损失函数为：
...
\begin{algorithm}[H]
\caption{基于 Deep Ritz Method 的多重特征值求解算法}
\label{alg:DRM_Eigen}
\begin{algorithmic}[1]
...
为了利用定义 \ref{def:staerr:pdim} 中定义的覆盖数来界定 Rademacher 复杂度，我们参考 Dudley 的经典结果。
...
基于上述引理，不妨记$\mathcal{N}^{1,2}$表示用于逼近梯度$\nabla u$的$ReLU$-$ReLU^2$神经网络函数类。结合引理\ref{lem:gradcomplexity}中给出的神经网络导数复杂度界，我们可以直接推导出该假设空间的伪维度上界为：
\begin{equation*}
...
\begin{remark}
从上述推论可以看出，空间维数 $d$ 仅存在于误差上界的常数项中，而并未出现在样本量 $n$ 的收敛阶（即指数项）的分母主导项中。这意味着，与有限元方法（FEM）等网格依赖型传统数值方法相比，本方法的计算复杂度不会随空间维数呈指数级爆炸，从而在一定程度上克服了“维数灾难”。然而需要审慎指出的是，在极高维情形下，上界中的常数项 $C(k,\dots)$的膨胀仍可能对算法的实际收敛表现产生实质性影响。
\end{remark}


\section{数值实验与超参数对解的影响}\label{sec_num} 
...
首先，我们选取不同的 $\mu_1$ 值并使用 Deep Ritz 方法进行求解。所得结果展示于图 \ref{fig:lambda1}。定理 \ref{mainthm} 指出，随着 $\mu_1$ 的增加，计算得到的特征值将趋近于真实值，实验结果验证了这一观察结论。
